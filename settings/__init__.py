"""settings
1, load variables from defaults module (defaults.py)
"""
import copy
import json
import logging
...
try:
    import defaults as default_settings
except ImportError:
    logger.debug(
        "Could't find default.py under %s directory. Skip loadding default", os.getcwd()
    )
...
def get_settings_priority(priority):
    ...
    :attr:`~settings.SETTINGS_PRIORITIES` dictionary and returns its
...
class BaseSettings(MutableMapping):
    ...
    already an instance of :class:`~settings.BaseSettings`, in which
    case the existing priority levels will be kept).  If the ``priority``
    argument is a string, the priority name will be looked up in
    :attr:`~settings.SETTINGS_PRIORITIES`. Otherwise, a specific integer
    should be provided.

    Once the object is created, new settings can be loaded or updated with the
    :meth:`~settings.BaseSettings.set` method, and can be accessed with
    the square bracket notation of dictionaries, or with the
    :meth:`~settings.BaseSettings.get` method of the instance and its
    ...
    def getdict(self, name, default=None):
        ...
        :class:`~settings.BaseSettings` instance itself, it will be
        converted to a dictionary, containing all its current settings values
        as they would be returned by :meth:`~settings.BaseSettings.get`,
    ...
    def maxpriority(self):
        ...
        :attr:`~settings.SETTINGS_PRIORITIES` if there are no settings
        stored.
        """
        if len(self) > 0:
    ...
    def set(self, name, value, priority="project"):
        ...
        :param priority: the priority of the setting. Should be a key of
            :attr:`~settings.SETTINGS_PRIORITIES` or an integer
    ...
    def setmodule(self, module, priority="project"):
        ...
        :meth:`~settings.BaseSettings.set` for every globally declared
    ...
    def setyaml(self, file, priority="project"):
        """update via yaml file"""
        self._assert_mutability()
        if not yaml:
            logger.error(
                "Couldn't import yaml, try to install it via `pip install pyyaml`"
            )

        with open(file, "r", encoding="utf-8") as f:
            config = yaml.load(f, yaml.FullLoader)
            self.update(config, priority)
    ...
    def update(self, values, priority="project"):
        ...
        This is a helper function that calls
        :meth:`~settings.BaseSettings.set` for every item of ``values``
        with the provided ``priority``.

        If ``values`` is a string, it is assumed to be JSON-encoded and parsed
        into a dict with ``json.loads()`` first. If it is a
        :class:`~settings.BaseSettings` instance, the per-key priorities
        will be used and the ``priority`` parameter ignored. This allows
        inserting/updating settings with different priorities with a single
        command.

        :param values: the settings names and values
        :type values: dict or string or :class:`~settings.BaseSettings`
...
class Settings(BaseSettings):
    ...
    :class:`~settings.BaseSettings`. Additionally, after instantiation
...
def overridden_settings(newsettings):
